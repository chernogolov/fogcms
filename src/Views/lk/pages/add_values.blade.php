<div class="col-12">
    <div class="container-input mb-4">
       <h1 class="blue">{{__('Add devices values')}}</h1>
       <form method="post" class="form-horizontal mt-5" id="sendValuesForm" enctype="multipart/form-data"
         xmlns="http://www.w3.org/1999/html">
       {{ csrf_field() }}
            @if($devices->total()>0)
                @foreach($devices as $device)
                   <div class="form-group row">
                       <label for="inputName" class="col-lg-7 col-form-label">{{$device->ServiceName}} <br class="d-block d-lg-none"><span class="text-muted">({{$device->MeteringDeviceModel}},&nbsp;{{$device->MeteringDeviceNumber}},&nbsp;{{$device->UnitName}})</span></label>
                       <div class="col-lg-5">
