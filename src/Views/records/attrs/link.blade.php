        ...
        <input autocomplete="off"
            ...
            @if(isset($attr->values) && count($attr->values)>0)
                @foreach($attr->values as $v)
                    ...
                    <input type="hidden" name="attr[{{ $attr->name }}][id]" value="{{ $v->id }}">
                @endforeach
                <select class="col-sm-6  col-xs-6 sl sl-r" data-id="{{$attr->meta}}" name="attr[{{ $attr->name }}][value]" id="list-{{$attr->meta}}" @if($attr->is_required == 1) required="required" @endif>
                    @foreach($attr->values as $v)
            ...
            @else
                <select class="col-sm-6 col-xs-6 sl sl-r clear" data-id="{{$attr->meta}}" name="attr[{{ $attr->name }}][value]" id="list-{{$attr->meta}}" @if($attr->is_required == 1) required="required" @endif></select>
            @endif
