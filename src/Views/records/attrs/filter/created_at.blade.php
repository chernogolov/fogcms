            ...
            <div class="modal-body">
                                ...
                                <div class="alert row">
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                            <label class="input-group-text" for="inputGroupSelect01">{{__('Created at')}}</label>
                                        </div>
                                        <input type="hidden"  name="filters[created_at][{{$i}}][]" value="records.created_at" form="modalForm">
                                        <select name="filters[created_at][{{$i}}][]" class="form-control" form="modalForm">
                                            <option value="=" @if($filter[1] == '=') selected @endif>=</option>
                                            <option value=">" @if($filter[1] == '>') selected @endif>&gt;</option>
                                            <option value="<" @if($filter[1] == '<') selected @endif>&lt;</option>
                                        </select>
                                        <input type="date" class="form-control" name="filters[created_at][{{$i}}][]" value="{{ $filter[2] }}"  form="modalForm">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-5 col-xs-8">
                        <input type="date" class="form-control" name="filters[created_at][{{$i}}][]" form="modalForm" value="{{ date('Y-m-d') }}">
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">{{__('Close')}}</button>
                <button type="submit" data-destination="records" class="btn btn-primary submit modal_form" form="modalForm">{{__('Filter')}}&nbsp;<span class="mdi mdi-filter"></span></button>
            </div>
        </div>
    </div>
</div>
